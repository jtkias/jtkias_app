...
- **The app link**: `https://jtkias.github.io/jtkias/` — appears in `thank-you.html` (the "Open the JTKias App" button) and `apps-script/Code.gs` (the `APP_LINK` variable, used in the confirmation email). It does not appear in `index.html` — that page's header button links to the on-page sign-up form (`#signup`), not directly to the app.
